...
export function getSofiaTime(): string {
...
export async function fetchFootballScores(
  ...
  const allScores: FootballScore[] = allFixtures
    .map((fixture, index) => {
      ...
      const odds = {
        home:
          oddsData.find((odd: OddsValue) => odd.value === "Home")?.odd ?? null,
        draw:
          oddsData.find((odd: OddsValue) => odd.value === "Draw")?.odd ?? null,
        away:
          oddsData.find((odd: OddsValue) => odd.value === "Away")?.odd ?? null,
      };
